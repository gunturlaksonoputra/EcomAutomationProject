...
import com.ecom.utility.NewExcelLibrary;
...
public class DataProviders {
	...
	//Class --> AddToCartPageTest, EndToEndTest,  Test Case--> addToCartTest, endToEndTest	
		...
		public Object[][] getRegisterData() {
			...
			// Totals rows count
			int rows = obj.getRowCount("RegisterData");
			// Total Columns
			int column = obj.getColumnCount("RegisterData");
			int actRows = rows - 1;
		...
		public Object[][] getEndToEndData() {
			// Totals rows count
			int rows = obj.getRowCount("EndToEndData");
			// Total Columns
			int column = obj.getColumnCount("EndToEndData");
			int actRows = rows - 1;
			...
			for (int i = 0; i < actRows; i++) {
				for (int j = 0; j < column; j++) {
					data[i][j] = obj.getCellData("EndToEndData", j, i + 2);
				}
			}
			return data;
		}
